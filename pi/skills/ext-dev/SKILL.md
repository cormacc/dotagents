...
- **Runtime validation** :: after adding, moving, or changing discovery of files under an extension auto-discovery root, start the installed `pi` binary in RPC mode and verify stderr contains no extension-load errors. Direct test imports and static metadata checks do not exercise Pi's factory discovery.
