...
Resolution is first-match-wins from `<ctx.cwd>/.agents/traits/`, when `ctx.isProjectTrusted()` is true, and then `~/.agents/traits/`. An untrusted project's layer is never passed to the interpolator. Each layer checks `<name>.md` before `<name>/prompt.md`; siblings such as `<name>/gate.md` are not candidates. The interactive consumer has no packaged layer. Fragment frontmatter is stripped and the body is substituted in place by the shared `herdr-orch.traits` implementation.
