...
Each candidate resolves through project `<git-root>/.agents/traits/` > home `~/.agents/traits/` > packaged `<skill-dir>/traits/`. Within each layer `<name>.md` wins over `<name>/prompt.md`; a match in a higher layer wins regardless of shape. The packaged directory is the skill-local relative symlink to the canonical trait store in this repository. Fragment YAML frontmatter is optional. When present, it is stripped before substitution and its `name:` must match the resolved file or directory name (`description:` and unknown keys are metadata only); when absent, the whole file is substituted verbatim.
...
`incompatible-with:` is optional fragment frontmatter containing a single-line, bare space-separated list of trait names. Each listed name must resolve in the normal trait layers whenever its declaring fragment resolves; a missing name fails at the same pre-allocation boundary as an unknown or repeated token. The relation is symmetric in effect: one declaration is sufficient, so when a listed trait also resolves in the same persona body the spawn fails naming the declaring fragment and both traits before task allocation, ledger/index creation, or pane mutation. Nested YAML and a second declaration mechanism do not exist.
