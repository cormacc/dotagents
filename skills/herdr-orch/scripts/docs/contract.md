...
| `ORCH_SETTLE_CLOSE_MS` | parent | Budget for the single `agent wait` any capture of a `COMPLETE`/`FAILED` envelope makes on the captured child before its one-shot pane close -- `run`, `collect <task>`, `collect --wait`, and `collect --any` alike (see § Ledger and completion). Unset, blank, unparseable, zero, and negative values all fall back to 45000 ms -- same discipline as `ORCH_POLL_INTERVAL_MS`. |
...
That close attempt probes `agent_status` exactly once and never retries, so a capture of a `COMPLETE`/`FAILED` envelope by the owning session first makes one bounded `agent wait` on the captured child -- and on no other child -- before it. This is a property of closure itself, so it applies identically to `run`, `collect <task>`, `collect --wait`, and `collect --any`; without it a child still mid-turn at the instant of capture keeps its pane forever, which a `collect` on the § Parent push advisory hits every time (the child is `working` for its whole notify wait). Never before the capture: the wait only buys the child time to reach `idle`/`done`, so a captured `BLOCKED` envelope, a foreign-session capture (neither closes a pane), and a `pending` collect (nothing captured) all skip it entirely. The budget is `ORCH_SETTLE_CLOSE_MS`, default 45 000 ms, deliberately larger than the 30 000 ms `--notify-timeout` default so that notify window is covered. The wait passes **no** `--until`, unlike § Parent push's, so any settled state ends it -- a child that settles `blocked` ends it immediately instead of burning the budget, and its pane is retained as always. Give-up is a non-event: budget expiry, a settled-but-unclosable state, or a failed `agent wait` all fall through to the same single close attempt, which simply finds the child unsettled and retains the pane. No field of the captured result, and not the exit code, depends on the wait or close outcome. The budget is *additional* to any polling budget: a capture can return up to the settle budget later than `--timeout` alone suggests (see § Fan-in).
...
The post-capture settle wait described in § Ledger and completion applies here unchanged, and `--timeout` is unaffected by it: that flag bounds only the polling for a publication, while the settle wait is a separate, additional, post-capture bound, so a capture landing just before the poll budget elapses can return up to `ORCH_SETTLE_CLOSE_MS` later than `--timeout` alone suggests. `--timeout` is still never overshot by a full poll interval. Every field of the result -- `remaining` included -- is a function of capture and candidacy alone, never of the close outcome: `remaining` is computed from the candidate set observed at capture, before the wait.
