...
A persona declares a trait by placing `%<name>` in its Markdown body where the fragment belongs; `traits:` frontmatter and a spawn-time trait flag do not exist. `herdr-orch.traits/interpolate` is the shared in-process implementation: callers inject the text, ordered resolution directories, existence predicate, and text reader, and receive transformed `:text`, ordered resolved `{trait, source, path}` maps, unresolved candidate names in `:unknowns`, and repeated resolving names in `:repeats`. Failure policy belongs to the caller. Phase 1 exposes no standalone interpolator CLI.
