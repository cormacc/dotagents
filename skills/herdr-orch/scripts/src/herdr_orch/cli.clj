...
;; forever even once every subsequent hook -- each wait tick, then a
    ...
    (loop []
      (if-let [parsed (capture! entry)] (finish-capture! entry parsed owned?)
          (let [remaining (- deadline (System/currentTimeMillis))]
            ;; A wait timeout is not a result: the child may still be working and publish
            ;; minutes later. Carry its live status so the caller can tell "still working"
            ;; from "gone" without a second manual `status` call -- the step whose omission
            ;; produced respawns that discarded finished work.
            (if (<= remaining 0) {:status "pending" :terminal? false :reason "timeout"
                                  :child-status (or (:agent_status
                                                     (try (herdr/agent! (:child entry))
                                                          (catch Exception _ nil)))
                                                    "unknown")
...
;; Ignored paths are deliberately distinct from dirty paths: the parent can rescue dirty
;; work before removal, while ignored build or scratch data is reported but never blocks it.
(defn ignored-paths [dir]
  (->> (str/split-lines (git! dir "status" "--porcelain" "--ignored=matching"))
...
(defn managed-worktree-root [] (fs/path (ledger/assignment-root) ".tmp" "herdr-orch" "worktrees"))
(defn worktree-checkout-path [task] (str (fs/path (managed-worktree-root) task)))
...
(defn- existing-worktree-target! [source requested]
  (let [target (canonical-checkout! "worktree target" requested)
        source-root (git-path! source "rev-parse" "--show-toplevel")]
    (if (= source-root target)
      ;; An explicit source-root target means that root is the requested pane cwd. An
      ;; omitted target never enters this function and preserves the caller's original cwd.
      {:kind :shared :path target}
      (do
        (assert-checkout-root! target)
        (let [repository (assert-same-repository! source-root target)]
  ...
  (let [source (source-cwd)
    ...
    (case (:kind decision)
      ...
      :existing (let [target (existing-worktree-target! source (:path decision))]
                  ...
                  (when (and (= :existing (:kind target)) read-only?)
                    (let [path (get-in target [:worktree :path])
                          dirty (dirty-paths path)]
                      (when (seq dirty)
                        (fail "worktree target refused: read-only child cannot use a dirty checkout"
                              {:path path :dirty dirty}))))
                  target)
      :new (let [task @task*
...
(defn- bounded-diagnostic [e]
  (let [message (or (ex-message e) (str e))]
    (subs message 0 (min 500 (count message)))))

(defn- inspect-reconciliation-worktree! [path repository]
  (let [checkout (canonical-checkout! "recorded worktree checkout" path)]
      ...
      (let [actual (git-path! checkout "rev-parse" "--git-common-dir")]
        (when-not (= repository actual)
          (fail "recorded worktree belongs to a different git repository"
                {:path checkout :recorded-repository repository
                 :actual-repository actual}))))
    checkout))

(defn reconciliation [entry]
  (when-let [{:keys [path branch base repository]} (:worktree entry)]
    (if-not (fs/exists? path)
      {:base base :branch branch :tip nil :checkout-state "missing"
       :committed nil :dirty nil :ignored nil}
      (try
        (let [checkout (inspect-reconciliation-worktree! path repository)
              tip (head-sha checkout)]
          ...
          {:base base :branch branch :tip tip :checkout-state "present"
           :committed (committed-paths checkout base tip)
           :dirty (dirty-paths checkout)
           :ignored (ignored-paths checkout)})
        (catch Exception e
          {:base base :branch branch :tip nil :checkout-state "invalid"
           :committed nil :dirty nil :ignored nil
           :diagnostic (bounded-diagnostic e)})))))
  ...
  (if (one opts :print-prompt)
    ...
    (do
      ...
      (let [path (roster persona)
            ...
            ;; Potentially blocking input and external inspection happen before reservation.
            ;; The lock protects only target decision and bounded local entry construction.
            assignment (task-text opts)
            own-persona (System/getenv "HERDR_ORCH_PERSONA")
            parent-label (:label (herdr/pane! (:parent-pane ident)))
            bin (launcher-bin)
            ;; The OS file lock serialises target decision and ledger reservation across
            ;; processes sharing this assignment root. It is released by the kernel on a
            ;; crash and before checkout creation or pane mutation begins.
            reservation
            (with-assignment-reservation-lock
              (fn []
                ;; The delayed UUID lets an explicit existing target complete canonicalisation,
                ;; repository, dirt, and live-reference validation before task allocation.
                (let [task* (delay (ledger/fresh-task))
                      target (resolve-worktree-target!
                              task* requested-target
                              (and (nil? requested-target)
                                   (worktree-in-flight? (:parent-session ident)))
                              read-only?)
                      task @task*
                      ...
                      label (core/child-label {:parent-label (when own-persona parent-label)
                                               :parent-persona own-persona :persona persona
                  ...
                  (ledger/write! entry)
                  {:target target :entry entry :worktree worktree :assignment assignment
                   :bin bin :name name :persona-path persona-path})))
            {:keys [target entry worktree assignment bin name persona-path]} reservation
            task (:task entry)
            result (:result entry)
            label (:label entry)]
        (try
...
(defn- assert-stream-capacity! [result]
  (let [items (bounded-result-items result)
        limit (max-stream-items)]
    (when (<= limit (count items))
      (fail "result stream exceeds item limit"
            {:result result :limit limit :count (count items)}))
    items))

(defn- append-publication! [result text]
  (let [items (assert-stream-capacity! result)]
...
(defn- inspect-checkpoint-worktree! [entry]
  (let [{:keys [path branch base repository] :as identity} (:worktree entry)
        _ (when-not (and (= #{:path :branch :base :repository} (set (keys identity)))
                         (every? some? [path branch base repository]))
            (fail "checkpoint publication requires complete worktree identity"
                  {:task (:task entry) :required [:path :branch :base :repository]
                   :present (vec (keys identity))}))
...
;; The staleness proof `prune` applies, shared verbatim with `orphans --prune` so the
;; foreign-owner path cannot drift into a weaker bar. Ownership is *not* part of it: that
;; is the one condition `orphans` deliberately overrides, and it is checked separately by
;; each caller. Returns nil when the entry is provably stale, or a [message data] refusal.
;;
;; `index-fn` is a thunk, not a value: the listing must run *after* the cheap ledger checks.
;; Calling it earlier lets a child that publishes inside the `agent list` window trip the
;; published-item check here instead of the final re-validation, which reports the race as
;; an ordinary stale refusal and loses the distinction the mutation guard exists to draw.
(defn- stale-round-refusal [entry index-fn]
  (let [task (:task entry)]
    (cond
      (:captured-at entry) ["assignment already captured" {:task task}]
      (contains? #{"failed" "invalid"} (:status entry))
      ["assignment is already terminal" {:task task :status (:status entry)}]
      (seq (:published (stream-state entry))) ["a published RESULT item already exists" {:task task}]
      :else
      ;; `nil` means the listing itself is unusable, so liveness is unknown: only a listing
      ;; that positively omits this child's name is proof of absence.
      (let [index (index-fn)]
        (cond
          (nil? index) ["agent list is unusable; liveness is unknown" {:task task}]
          (contains? index (:child entry)) ["named child is present in agent list" {:task task :child (:child entry)}]
          :else nil)))))
...
(defn prune! [task]
  ...
  (let [entry (ledger/read! task)
    ...
    ;; second liveness scan.
    (when-let [[message data] (stale-round-refusal entry live-agents)]
      (fail (str "prune refused: " message) data))
...
(defn orphans! [opts]
  ...
  (let [close? (boolean (one opts :close))
        prune? (boolean (one opts :prune))
        _ (when (and close? prune?)
            (fail "orphans takes --close or --prune, not both" {}))
        ...
        ;; `--prune` takes the mirror set of the listing/`--close` set: uncaptured rounds
        ;; rather than captured ones. A foreign session's stale entry was previously an
        ;; uncontrolled manual recipe (delete the ledger JSON after eyeballing `agent list`),
        ;; which is the same weak-evidence step `orphans` already replaced for panes.
        candidates (->> (ledger/entries)
                        newest-rounds
                        (filter #(and (if prune?
                                        (not (stream-captured? (stream-state %)))
                                        (and (stream-captured? (stream-state %)) (:pane-id %)))
    ...
    (cond
      prune?
      ;; Ownership is the single condition the operator overrides. Every other part of
      ;; `prune`'s proof applies unchanged, including the final re-validation inside
      ;; `ledger/update!` that refuses a capture appearing during the listing.
      (mapv (fn [entry]
              (let [task (:task entry)]
                (if-let [[message data] (stale-round-refusal entry live-agents)]
                  {:status "refused" :task task :child (:child entry) :reason message :detail data}
                  (do (ledger/update! task
                                      (fn [current]
                                        (if (or (:captured-at current) (seq (:published (stream-state current))))
                                          (fail "orphans --prune refused: assignment was captured or published during the liveness check"
                                                {:task task})
                                          (assoc current :status "failed" :failure-phase "orphaned"
                                                 :pruned-at (now) :prune-reason "missing-agent"
                                                 :pruned-by "orphans"))))
                      {:status "pruned" :task task :child (:child entry)
                       :parent-session (:parent-session entry)}))))
            candidates)
...
;; "Teardown" decision).
;;
;; The hard guards are managed-root containment, dirt, and liveness. Ownership follows `prune!`/`close`'s rule
...
(defn resolve-worktree-path [path]
  (loop [candidate (fs/normalize (fs/absolutize (fs/path path))) suffix ()]
    (if (fs/exists? candidate)
      (reduce (fn [resolved segment] (fs/path resolved segment))
              (fs/real-path candidate) suffix)
      (recur (fs/parent candidate) (conj suffix (fs/file-name candidate))))))
;; This predicate takes an already-resolved path so later worktree-target interfaces can
;; apply the same containment rule without coupling it to their argument handling.
(defn within-managed-worktree-root? [resolved-path]
  (fs/starts-with? resolved-path (resolve-worktree-path (managed-worktree-root))))
(defn- assert-managed-worktree-path! [task path]
  (let [resolved-path (resolve-worktree-path path)]
    (when-not (within-managed-worktree-root? resolved-path)
      (fail "worktree remove refused: recorded checkout is outside the managed root"
            {:task task :path path :resolved-path (str resolved-path)
             :managed-root (str (resolve-worktree-path (managed-worktree-root)))}))))
(defn- worktree-of [verb entry]
  (or (:worktree entry) (fail (str verb " refused: this task recorded no worktree") {:task (:task entry)})))
...
(defn worktree-remove! [task]
  ...
  (let [entry (ledger/read! task)]
    (assert-worktree-owned! "worktree remove" (caller-parent-session) entry)
    (let [{:keys [path branch base]} (worktree-of "worktree remove" entry)]
      (assert-managed-worktree-path! task path)
      ...
      (if-not (fs/exists? path)
        ...
        (let [dirty (dirty-paths path) ignored (ignored-paths path)]
          (when (seq dirty)
            (fail "worktree remove refused: checkout is dirty" {:task task :path path :dirty dirty :ignored ignored}))
          (let [tip (head-sha path) ancestor? (ancestor-of-parent-head? (source-cwd) tip)]
            ...
            {:status "removed" :task task :child (:child entry) :path path :branch branch :base base
             :tip tip :tip-is-ancestor-of-parent-head ancestor? :ignored ignored}))))))
...
;; `poke` re-issues the publication instruction to a settled child that finished its work
;; without publishing, or wrote prose to RESULT instead of an envelope. It is the recovery
;; verb for the measured case where a parent otherwise adopts unvalidated pane text or pays
;; twice by respawning. It creates no round, allocates nothing, and touches no pane: the
;; only mutation is the prompt itself, so a refused poke leaves the round exactly as it was.
(defn poke-prompt [task result]
  (str "Your round is not published. Herdr assigned TASK=" task " and RESULT=" result ". "
       "Publish now with `$HERDR_ORCH_BIN task publish --task " task
       " --status COMPLETE --summary \"...\"`, or `--status BLOCKED`/`--status FAILED` if you cannot finish. "
        ...
        (with-child-lock
         ...
         (fn []
           (let [current (ledger/read! task)]
             ...
             (herdr/prompt! child (poke-prompt task (:result current)))
    ...
    (with-assignment-reservation-lock
     (fn []
      (let [entry (ledger/read! prior-task)
            ...
            newest (newest-round entries child)
            worktree-path (get-in entry [:worktree :path])
            other-entries (remove #(= prior-task (:task %)) entries)]
        ;; Repeat every mutable ledger guard after the settle wait and under the same lock
        ;; used by explicit starts. Whichever contender writes first becomes visible to the
        ;; other's guard before that contender can reserve its own round.
        (when (seq (unconsumed-items entry))
          (fail "continue refused: an item was published during the settle wait"
                {:task prior-task}))
        (when-not (and (stream-captured? state)
                       (not (contains? #{"failed" "invalid"} (:status entry))))
          (fail "continue refused: the prior round changed while settling"
                {:task prior-task :status (:status entry)}))
        (when (:closed-at entry)
          (fail "continue refused: this child's pane was closed while settling"
                {:task prior-task :closed-at (:closed-at entry)}))
        (when-not (= prior-task (:task newest))
          (fail "continue refused: a newer round exists for this child"
                {:task prior-task :child child :newest (:task newest)}))
        (when (and worktree-path
                   (worktree-referenced-by-live-round? worktree-path other-entries))
          (fail "continue refused: another live round references this checkout"
                {:task prior-task :child child :path worktree-path}))
  ...
  (let [assignment (task-text opts)
    ...
    (let [ident (try (parent-identity) (catch Exception _ nil))
      ...
      (let [{:keys [index]} (settle-and-list! "continue" entry)
        ...
        ;; Slow settling and every Herdr call stay outside the reservation. The helper
        ;; re-reads and re-validates mutable ledger state, then writes the successor while
        ;; explicit starts are excluded by the same assignment-root lock.
        (let [{:keys [task result]}
              (reserve-continuation! prior-task child ident caller waiting-policy)]
