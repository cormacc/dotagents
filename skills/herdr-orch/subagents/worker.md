...
- Commit policy depends on `HERDR_ORCH_WORKTREE`, the env var `oh` injects only when this round runs inside its own git worktree checkout. When it is set, committing on that worktree's branch is your default, not an exception: the rule protecting the caller's tree protects nothing on a branch cut just for you, and the branch itself is the deliverable the caller merges. Commit your work there before you finish, even if the assignment never says the word "commit", loading the commit-message skill the installation provides (in this repository, `git-commit`) and following repository commit conventions; leave merging, rebasing, or pushing over another branch to the caller. When `HERDR_ORCH_WORKTREE` is absent, you are in the caller's shared tree: do not commit unless the assignment explicitly requests a commit. When requested there, load that same commit-message skill and follow repository commit conventions.
