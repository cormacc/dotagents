...
No capture closes a pane -- not `run`, `collect`, `collect --wait`, or `collect --any`, and not for any envelope status. `close <full-task-uuid>` is the only path that does: it acts only on a pane this parent created, only from that child's newest round, and only on a live observation matching both the recorded child name and its pane ID in `idle`/`done`. Never close user/other-agent panes, kill a parent, or stop the Herdr server. A different parent session may collect and validate an assignment but can neither close nor continue it -- that decision belongs to the owner.
