...
A child that publishes *nothing* is a separate case: its ledger entry stays `prompted` with no `RESULT` file even though the work may be finished. When such a child has settled, `oh task poke <full-task-uuid>` re-issues the publication instruction before considering a respawn -- respawning discards completed work and pays for it twice.
