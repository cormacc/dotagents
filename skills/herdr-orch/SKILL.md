...
`oh` also wraps the raw Herdr `pane`, `tab`, `ws`, and `agent` verbs, plus `oh spawn "<shell command>"` for an ordinary command in a new tab, always unfocused because it never delegates (unlike `task run`/`start` -- see Focus below), each returning the same JSON envelope; run `oh --help` for the verb list. The upstream Herdr safety rules apply to those exactly as they do to direct `herdr` use. The wrapper is not a transparent mirror, though: it imposes agent-facing defaults that differ from upstream, so pass the flags explicitly when upstream semantics matter. `oh agent wait` defaults to a 600 s timeout where `herdr` waits indefinitely, and the read family defaults to `--source recent-unwrapped` (upstream: `recent`), falls back to `visible` on empty output, and truncates to 2000 lines / 50 KB.
...
**Harness is a deployment property, model is a per-assignment dial.** There is no spawn-time harness flag: the child runs whatever its definition's `kind:` declares, or -- far more usually -- the harness Herdr measured for you. `--model` is the only tier control, and a model name (including a weight alias) never selects a harness. The four shipped weights are `heavy`, `middle`, `light`, and `feather`; each is translated for the already-resolved kind through the `config.edn` chain, so `--model light` is the portable way to ask for a tier. Per-harness spellings, chain precedence, the two-level `:aliases`/`:models` split and its merge semantics (a `:models` row is replaced wholesale, while `:aliases` composes per key so an override retargets a weight in one line), floating-versus-pinned IDs, pass-through of unknown IDs, and `:extra-args` (including how an override relaxes a harness's interactive approval so unattended children do not stall) all live in the [contract](scripts/docs/contract.md) -- read it when authoring or debugging an override, not when delegating.
...
`continue` is root-only, and its preconditions are mechanical guards rather than advice: it refuses, mutating nothing, unless you own the entry, that round was captured with a validated `COMPLETE`/`FAILED`/`BLOCKED` envelope, no other round of that child is still open, it is that child's newest round, and the live agent's name **and pane ID** match the ledger in `idle`/`done`. A `working`, `blocked`, vanished, or pane-mismatched child is refused, not reused.
...
No capture closes a pane -- not `run`, `collect`, `collect --wait`, or `collect --any`, and not for any envelope status. `close <full-task-uuid>` is the only path that closes a child that actually started work (spawn-failure cleanup aside, which takes a pane the child never worked in): it acts only on a pane this parent created, only from that child's newest round, and only on a live observation matching the recorded child name and pane ID in `idle`/`done` (or, once released, the fallback below). Never close user/other-agent panes, kill a parent, or stop the Herdr server. A different parent session may collect and validate an assignment but can neither close nor continue it -- that decision belongs to the owner.

`close` reports what it did: `closed` (pane taken, `:closed-at` recorded), `retained`/`unsettled` (a live occupant isn't settled -- retry), or `gone` (`pane-absent`; or live but unconfirmed `name-released`, with a `remedy`). An absent name falls back to the shell_pid recorded at spawn before reporting `gone`, so a match closes a resumed child (contract.md § Close). Only `closed` means the pane is gone. Guard violations -- a foreign or unresolvable caller, an uncaptured entry, a stale round, a pane the child no longer occupies -- fail loudly and change nothing. `close --settled` instead returns one outcome per child and exits 0 even when some refused, so read its array rather than its exit code. Both `close` and `continue` first make one bounded `ORCH_SETTLE_CLOSE_MS` wait (default 45 s), because a child that has just published is usually still `working`.

A child whose owning parent session has ended cannot be closed through `close` at all: ownership is absolute and has no dead-owner exception, because no signal reliably distinguishes a dead session from a live one. Such an orphan is an operator cleanup: `orphans` lists them, and `orphans --close` sweeps them under the same live child+pane match. Both forms are root-only, like `continue`, and both refuse an unresolvable caller: below root every other session looks foreign, and with no caller identity even your own children list as orphans.

Never bulk-delete the ledger, and do not treat a retro or session end as a cue to tidy it. It is shared by every session using the same assignment root, so its entries are not yours to remove: `prune` refuses an entry another session owns for the same reason `close` does, and a directory that looks like stale scratch may hold a concurrent session's in-flight state. It is also the only durable route to a finished child -- `:child-session` outlives the pane, and a task UUID cited in a commit, record, or task body resolves nowhere once its entry is gone. Retire your own entries with `prune` (a stale one) or `compact` (drops envelope bulk from a closed round, keeping the references a cited UUID resolves through), and read this session's process candidates with `harvest` rather than deleting what you do not want to read again.
...
Use caller context or explicit IDs, and response IDs -- never focused UI state, which the Focus rules above only ever *set*. Labels never contain a workspace name and never replace unique agent names. Nested labels depend on the spawning agent's injected `HERDR_ORCH_PERSONA`; a spawning persona started outside `oh` has no nested-label identity unless that variable is set.
...
One validator's approval is weak evidence on a subtle surface. Measured on this skill's own lifecycle rewrite: two reviewers given an identical assignment on different models returned opposite verdicts -- one APPROVED with no P0-P2, the other four P2 defects all confirmed at source -- and their findings overlapped on a single line of one file. For a Class C closeout, prefer two independent validators and expect disjoint findings; treat a lone APPROVED as unfalsified rather than validated.
