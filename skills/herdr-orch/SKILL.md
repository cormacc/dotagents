...
- **Placement:** explicit `--tab` or `--split` overrides configured `:defaults :placement`, which ships as `:tab-split` -- a root-level child gets its own tab (reviewable and recoverable when you spawn freely), a below-root child splits its parent's pane; `:split` and `:tab` force one geometry at every depth. Tab placement creates the child in a new unfocused tab of the caller's workspace; every other contract (env, label, ledger, collect, closure) is identical. Placement is never persisted per-child or inherited via env: a child's own spawns resolve from config and depth alone.
...
- **Lifecycle:** a child's pane persists until you act on it. Capture closes nothing, so after every capture make the decision explicitly: `close <full-task-uuid>` ends the child, or `continue <full-task-uuid> --task '<next assignment>'` gives it another round in the context it already holds. `close --settled` sweeps every settled, captured child of your own session in one call. The child is never told which you chose and never chooses for itself.

Whoever spawns a child closes it. `close` refuses an entry another session owns, and `close --settled` skips it, so a grandchild belongs to the child that spawned it -- a delegating persona must close its own children before publishing, or leave a pane nobody in the protocol can take.
...
No capture closes a pane -- not `run`, `collect`, `collect --wait`, or `collect --any`, and not for any envelope status. `close <full-task-uuid>` is the only path that closes a child that actually started work (spawn-failure cleanup aside, which takes a pane the child never worked in): it acts only on a pane this parent created, only from that child's newest round, and only on a live observation matching both the recorded child name and its pane ID in `idle`/`done`. Never close user/other-agent panes, kill a parent, or stop the Herdr server. A different parent session may collect and validate an assignment but can neither close nor continue it -- that decision belongs to the owner.

`close` reports what it did rather than leaving you to assume it: `closed` (pane taken, `:closed-at` recorded), `retained`/`unsettled` (the child is live but still working -- retry), or `gone`/`child-absent` (the agent name was released, so the pane is deliberately left alone; it may now host something else). Only `closed` means the pane is gone. Guard violations -- a foreign or unresolvable caller, an uncaptured entry, a stale round, a pane the child no longer occupies -- fail loudly and change nothing. `close --settled` instead returns one outcome per child and exits 0 even when some refused, so read its array rather than its exit code. Both `close` and `continue` first make one bounded `ORCH_SETTLE_CLOSE_MS` wait (default 45 s), because a child that has just published is usually still `working`.

A child whose owning parent session has ended cannot be closed through `close` at all: ownership is absolute and has no dead-owner exception, because no available signal distinguishes a dead session from a live one reliably enough to risk closing another parent's child. Such an orphan is an operator cleanup -- confirm it with `oh agent list`, then `oh pane close <pane-id>` -- exactly as a foreign session's stale ledger entry already is.

Never bulk-delete the ledger, and do not treat a retro or session end as a cue to tidy it. It is shared by every session using the same assignment root, so its entries are not yours to remove: `prune` refuses an entry another session owns for the same reason `close` does, and a directory that looks like stale scratch may hold a concurrent session's in-flight state. It is also the only durable route to a finished child -- `:child-session` outlives the pane, and a task UUID cited in a commit, record, or task body resolves nowhere once its entry is gone. Retire your own stale entries with `prune`, and scope a candidate harvest by session rather than by deleting what you do not want to read again.
