...
A persona may additionally declare `requires:` — the subset of its own `spawns:` allow-list whose consult its documented workflow *mandates* (the packaged `worker` declares `requires: advisor` for its pre-publish review). It is parsed like `spawns:`, is not overridable by a flag, and is validated against the persona's declared `spawns:`, so a mandate naming a persona it cannot spawn fails fast at any depth. The validated result is then narrowed to the effective allow-list and recorded as `:required`: `--spawns none` and a depth-forced leaf both withdraw the mandate rather than emit an impossible instruction. In the composed guidance a non-empty `:required` replaces the single may-spawn sentence with a mandate clause (perform it at the points the persona defines, not only when blocked), an optional gap-only clause for the remaining allow-list, and one shared bound sentence — so a mandated consult is never read as forbidden by the gap-only trigger, while one-at-a-time, blocking, ephemeral, and leaf still apply to both kinds. `enforce-spawns!` remains the sole mechanical gate; `requires:` is prompt semantics, not extra capability.
