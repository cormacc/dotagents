...
**Use `clj-paren-repair` for file repair even when pi-clojure is loaded** — it uses a real Clojure reader (edamame), parinfer-rust, and cljfmt, so the output is also formatted. `clojure_paren_repair` (JS parinfer) is for string repair before writing a new file. Never hand-fix delimiter errors.
...
- Babashka resolves `user.home` from the OS user database, not `$HOME`: in bb subprocess tests, never rely on a `$HOME` override to isolate home-directory probes — inject an explicit root/path override instead.
