...
flowchart TD
    ...
    subgraph Core
        ...
        PARSER["parser.clj\nstable facade + scanner/serializers"]
        PARSERINT["parser/{timestamps,properties,links,issues}.clj\nfocused parser helpers"]
        EFF["effective.clj\n#+SETUPFILE: chains"]
...
- =org-tasks.parser= is the stable public facade: it owns the stateful line scanner, task-tag operations, and serializers, while =parser.timestamps=, =parser.properties=, =parser.links=, and =parser.issues= own focused helpers. =lifecycle= consumes timestamp/property helpers directly, so its =closed-statuses= is canonical without a parser cycle. The scanner retains round-trip guarantees: unknown keywords, drawers, and body content are preserved byte-identically (fixtures under =test/fixtures/round-trip/=). An unterminated =:PROPERTIES:=/=:LOGBOOK:= drawer (no =:END:= before EOF) is a hard parse error (=unterminated-drawer=, naming file + line) rather than silently swallowing the rest of the file.
...
The same shape applies to =priority=, =select=, =remove=, =archive=, =publish=, and the property commands (=handoff= / =blocker= / =issue=, generated from a per-property data map in =commands/links.clj=). =commands/removal.clj= computes pure subtree/inbound-blocker impact before confirmation, then uses =tree/remove-by-id= and source-root saving; it never lets a protocol top-level root bypass lifecycle plus archive history. Cycle orders (status, priority incl. the unset slot) live in =lifecycle.clj=; both UIs pass only a direction.
...
skills/org-tasks/scripts/
  ...
  src/org_tasks/
    ...
    parser.clj                  # stable facade; scanner/serializers (round-trip preserving)
    parser/                     # timestamps, properties, links, linked issues
