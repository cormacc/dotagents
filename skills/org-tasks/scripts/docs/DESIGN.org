...
flowchart TD
    ...
    subgraph Commands
        ...
        FAM["commands/{list_show,create,move,removal,status,\narchive_publish,record,links,maintenance}.clj"]
        UTIL["commands/util.clj\ncontext + id resolution"]
    end
    subgraph Core
        LOADER["loader.clj\ngraph load/save, atomic IO"]
        PARSER["parser.clj\nstable public facade"]
        PARSERINT["parser/{scanner,renderer,lines,timestamps,properties,links,issues}.clj\nfocused parser helpers"]
        READY["readiness.clj\nblocker graph evaluation"]
        EFF["effective.clj\n#+SETUPFILE: chains"]
...
- =org-tasks.parser= is the stable public facade. =parser.scanner= owns task headings and the lossless task-tree scan. =parser.lines/next-block-kind= and =parser.lines/block-boundary?= provide the shared lexical block-boundary rule for task, section, and doctor scans. =parser.renderer= owns canonical whole-root rendering and locality field patches. The facade retains the baseline =:doc= and =:arglists= metadata for every pre-extraction public var. =readiness= owns blocker graph evaluation. =parser.timestamps=, =parser.properties=, =parser.links=, and =parser.issues= own focused helpers. =lifecycle= consumes timestamp/property helpers directly, so its =closed-statuses= is canonical without a parser cycle. The scanner derives accepted heading states from =lifecycle/status-cycle=. Unknown keywords, drawers, and body content remain byte-identical in unchanged source regions. An unterminated =:PROPERTIES:=/=:LOGBOOK:= drawer (no =:END:= before EOF) is a hard parse error (=unterminated-drawer=, naming file + line) rather than silently swallowing the rest of the file.
- Each UUID has one canonical writable node: mutations on imported plan tasks persist to the task's own source file via =loader/save-source-roots=, never to a copy. =archive=/=publish=/=unpublish= re-stamp a task's =:source-path= and hand the whole graph to =save-source-roots=, so it works uniformly whether the task's original root lived in =TASKS.org=, =TASKS.local.org=, or a file-level =#+IMPORT:= plan file -- it is never left behind in one file while appearing in another. =unarchive= is the guarded inverse: it resolves only the archive file, preflights active/archive/record baselines, restores into the explicit or stamped level-1 section, and leaves lifecycle state untouched.
- =save-source-roots= is an optimistic-concurrency writer: before the first rewrite it preflights every known affected baseline against the load-time snapshot (=:source-content=, or a caller-supplied baseline for files that may end up with zero roots), then atomically replaces each changed source. A detected conflict aborts before any write; a race after preflight remains outside the filesystem-transaction boundary. Every file write goes through =loader/atomic-write= (unique per-call tmp name, then rename).
...
sequenceDiagram
    ...
    H->>IO: save-source-roots-locality (atomic tmp+move)
...
=move= is the one structural mutator that bypasses the serializer: relocating a subtree changes nothing but heading depth, so =commands/move.clj= cuts the subtree's exact source lines, restars its headings, and splices them at the destination (reusing =insert/insert-subtree-into-section= for a =--section= target). That keeps drawers, LOGBOOK, bodies, descendant order, *and* intra-subtree blank lines byte-identical, and leaves every unrelated region of the file untouched rather than putting it through =serialize-tasks-preserving-file='s blank-line normalisation. It is in-file only by construction -- source and destination must share one =:source-path= -- so no UUID can gain a second writable node; locality and archive transitions stay with =publish=/=unpublish= and =archive=/=unarchive=.

In-place =status=, =priority=, and property commands (=handoff= / =blocker= / =issue=) use the locality renderer. It patches changed headings, lifecycle lines, and drawer ranges. New fields use the canonical order: =CLOSED:= after the heading, then =:PROPERTIES:=, =:LOGBOOK:=, and =#+IMPORT:=. Existing field ranges update in place. It retains every other byte in the changed task and all unrelated source bytes. Creation, removal, archive, restore, publish, unpublish, and record migration retain canonical whole-root rendering. Both paths use the same optimistic-concurrency preflight and atomic writes. =commands/removal.clj= computes pure subtree/inbound-blocker impact before confirmation, then uses =tree/remove-by-id= and source-root saving; it never lets a protocol top-level root bypass lifecycle plus archive history. Cycle orders (status, priority including the unset slot) live in =lifecycle.clj=; both UIs pass only a direction.
...
skills/org-tasks/scripts/
  ...
  src/org_tasks/
    ...
    parser.clj                  # stable public facade
    parser/                     # scanner, renderer, lines, timestamps, properties, links, linked issues
    readiness.clj               # blocker graph evaluation
