...
- **Contract-bound machine output.** Every key, field, and value in `--format json|edn` output is contract: [`docs/contract.md`](docs/contract.md). Behaviour changes require a contract update in the same change; silent envelope drift is a P1 regression (it has happened — mutation-command envelopes are now golden-tested).
...
- **Parity**: user-visible mutations should stay available in both the TUI and the pi overlay (`pi/extensions/tasks/`), each dispatching through the same `ot` command. Ownership split: [`docs/boundary.md`](docs/boundary.md).
- **Skill docs**: user-visible CLI/TUI changes must update [`../references/ot-cli.md`](../references/ot-cli.md) (and [`../SKILL.md`](../SKILL.md) when workflows change) in the same change — `ot doctor`'s spec check only warns when a record declares the path.
