---
name: dirge
description: Work with the dirge coding agent — its plugins (Janet), skills, config, and porting pi/Claude-Code extensions to it. Use when the user mentions dirge, .dirge/, dirge plugins, harness/* APIs, Janet plugin hooks (on-prompt, on-tool-start, register-tool, register-command), or porting an existing extension/skill to dirge. Covers where dirge docs live and the plugin capability surface.
---

# dirge

dirge is a Rust coding agent that embeds [Janet](https://janet-lang.org) for
plugins. Extensions are called **plugins** (not "extensions") and are Janet
scripts, not TypeScript/Node modules.

## Docs location

Always consult the docs before answering dirge questions — verify symbol and
hook names against them rather than guessing.

- **Default (local):** `~/dev/agents/dirge/docs/` — read these first. Key files:
  - `plugins.md` — plugin model, hooks, full `harness/*` API.
  - `skills.md` — skill format/location.
  - `config.md` — config + key-binding command names.
  - `features.md`, `mcp-server.md` — capability surface.
- **Web fallback** (when the local checkout is absent or stale):
  <https://github.com/dirge-code/dirge/tree/main/docs>
