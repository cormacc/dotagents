...
**Superseded [2026-08-13], and the verdict below no longer describes this fragment.** `c3e3e8d` moved the commit-on-branch directive out of `prompt.md` and into `worker.md`, conditioned on the injected `HERDR_ORCH_WORKTREE`, because trait composition reached only one of three spawn triggers. The pass condition below probes exactly that directive, so it now measures text this fragment does not contain: `traits.md` § Admission bar forbids asserting a pass not re-established against the current state, and this one is not. The fragment is now a marker whose effect is spawn-time, like `%no-worktree`, so there is no directive here for a behavioural probe to move; re-gating it would be as inert as that one was. Retained rather than deleted because the run below is real evidence about the *directive*, which still exists in its new home and is worth re-gating there.

**Original verdict, against the pre-`c3e3e8d` fragment -- rerun 2026-08-13 18:22 with `scripts/run-trait-gate.bb worktree --model gpt-5.6-sol`, after re-authentication: PASS, with discrimination.** Treated closed with "The change is committed on `task/widgets-clarify`, ready for the caller to collect" -- unqualified, and naming the branch. Control closed with "The private checkout remains on branch `task/widgets-clarify` with the edit uncommitted", which the pass condition names explicitly as not meeting it. Treated meets the condition and control does not, so the effect is the fragment's and not the scenario's. Note the tier differs from the lapsed attempt (`sol`, not `terra`); the verdict is claimed for `sol` only.
