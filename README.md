...
`pi/settings.json`'s `packages` list carries the third-party `git:github.com/DietrichGebert/ponytail` package as an object entry rather than a bare source string, using pi's own per-package resource-selection filter (`pi config`'s TUI writes the same shape) to force-exclude its `pi-extension/index.js` (the automatic full-mode system-prompt injection) and its `ponytail`, `ponytail-review`, and `ponytail-audit` skills (respectively: the broad "use on any coding task" trigger, and the two skills whose review/audit triggers overlap `skills/complexity-review`'s own routing). This replaces automatic Ponytail activation with the composed `simple`/`prune` traits (`skills/herdr-orch/traits/`) and the `complexity-review` skill for the workflows those three skills covered; `ponytail-debt`, `ponytail-gain`, and `ponytail-help` are untouched; nothing in the ponytail package cache itself is edited. The excluded extension and skills stay installed and readable as upstream reference/comparison material at `/home/cormacc/.pi/agent/git/github.com/DietrichGebert/ponytail/` (a stable path once the package is installed); load one deliberately for a single run with `pi --skill <that path>/skills/ponytail --extension <that path>/pi-extension/index.js`, which works whether or not the package's own resources are filtered, or flip the filter for a session with `pi config` (Tab switches global/project scope).
